...
def upload_RiskReport(value):
    ...
    elif valueRisk>0 and valueRisk<1:
        messageToDisplay ="El Nivel de Riesgo Medio, se encontró un lugar y hora común con un caso confirmado (Risk Score=" + str(math.floor(valueRisk)) + ")"
    elif valueRisk>1:
        messageToDisplay ="El Nivel de Riesgo Alto, se encontraron lugares y horas en común con casos infectados (Risk Score=" + str(math.floor(valueRisk)) + ")"
...
def CheckRiskPlace(conn ,Lat,Long,InitT,FinalT,Conf,PID) :
    ...
    for x in myresult:
        LatCase=x[0]
        LongCase=x[1]
        Distance=ComputeDistanceLongLat(float(Lat)/10000000,float(Long)/10000000,float(LatCase)/10000000,float(LongCase)/10000000)
        # print(Distance)
        if Distance<=100:
            Score=Score+ (1/(1+math.exp(-0.1*(50-Distance))))
        # print(Score)
    return Score
...
def CheckCoincidencesinBD(filename):
    ...
    # else:
    #     startingLocation = [19.432608, -99.133209]
...
def InsertDataToBD(filename):
    ...
    for i in range(len(data['timelineObjects'])):
        ...
        if data['timelineObjects'][i].get('placeVisit'):
            ...
            # print(datetime.fromtimestamp(float(EndTime)/1000.0))
            NumberOfVisitedPlaces=NumberOfVisitedPlaces+1
            Place = [float(data['timelineObjects'][i]['placeVisit']['location']['latitudeE7'])/10000000 , float(data['timelineObjects'][i]['placeVisit']['location']['longitudeE7'])/10000000,1000]
            ListOfPlaces.append(Place)
